...
class ResourceItemSerializer extends EsSerializer {
  serialize (options) {
    ...
    ; ['Volume Range', 'Date Range'].forEach((name) => {
      itemFieldMapper.getMapping(name, (spec) => {
        this.object.each(spec.pred, (triple) => {
          ...
          if (utils.isArrayWithValues(range)) {
            this.addStatement(spec.jsonLdKey, range)
          }
        })
      })
    })
    ...
    // Add `type` of 'bf:Item' (differentiated from checkin card items,
    // which have type 'nypl:CheckinCardItem')
    itemFieldMapper.getMapping('Type', (spec) => {
      this.addStatement(spec.jsonLdKey, 'bf:Item')
    })

    // Add bib's resource type (aka material type) as the item's formatLiteral.
    // For serials, this formatLiteral will co-mingle with holding.format
    // values (e.g. Print, Microfilm, Book set)
    itemFieldMapper.getMapping('Format literal', (spec) => {
      ...
      if (bibResourceType && bibResourceType.object_label) {
        this.addStatement(spec.jsonLdKey, bibResourceType.object_label)
      }
    })
