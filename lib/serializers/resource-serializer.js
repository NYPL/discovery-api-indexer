...
class ResourceSerializer extends EsSerializer {
  serialize () {
    ...
    if (this.object._items) {
      ...
      let numElectronicResources = 0
      this.object._items.forEach((item) => {
        if (item.has('bf:electronicLocator')) {
          numElectronicResources = item.statements('bf:electronicLocator').length
      ...
      promises.push(Promise.all(this.object._items.map((item) => ResourceItemSerializer.serialize(item, this.object))).then((itemsSerialization) => {
        ...
        this.addStatement('numItems', itemsSerialization.length - numElectronicResources)
