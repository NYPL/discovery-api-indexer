  ...
  serialize () {
    ...
    if (this.object._items) {
      ...
      promises.push(Promise.all(this.object._items.map((item) => ResourceItemSerializer.serialize(item, this.object))).then((itemsSerialization) => {
        ...
        const itemsWithElectronicResources = itemsSerialization.filter((item) => item.electronicLocator)
        this.addStatement('numItems', itemsSerialization.length - itemsWithElectronicResources.length)
