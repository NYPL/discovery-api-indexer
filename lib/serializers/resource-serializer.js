...
class ResourceSerializer extends EsSerializer {
  serialize () {
    ...
    const titleSpec = fieldMapping.getMapping('Title')
    if (this.object.has(titleSpec.pred)) {
      const title = this.object.statement(titleSpec.pred).object_literal
    ...
    if (this.object.has(fieldMapping.predicateFor('Contributor literal'))) {
      ...
      const contributor = this.object.statement(fieldMapping.predicateFor('Contributor literal')).object_literal
    ...
    if (this.object.has(fieldMapping.predicateFor('Creator literal'))) {
      ...
      const creator = this.object.statement(fieldMapping.predicateFor('Creator literal')).object_literal
    ...
    const defaultShelfMark = this.object.literals('nypl:shelfMark')
    ...
    if (this.object._items) {
      ...
      promises.push(Promise.all(this.object._items.map((item) => ResourceItemSerializer.serialize(item, this.object))).then((itemsSerialization) => {
        ...
        // Calculate number of "real" sierra items (not checkin card items,
        // not electronic items)
        const numRealSierraItems = itemsSerialization
          .filter((item) => !item.electronicLocator)
          .filter((item) => item.type && item.type[0] === 'bf:Item')
          .length
        this.addStatement('numItems', numRealSierraItems)

        // Calculate number of electronic resources
        const itemsWithElectronicResources = itemsSerialization
          .filter((item) => item.electronicLocator)
        const numElectronicResources = itemsWithElectronicResources
          .reduce((total, currentItem) => currentItem.electronicLocator.length + total, 0)
        this.addStatement('numElectronicResources', numElectronicResources)

        // Calculate number of checkin card items:
        const numCheckinCardItems = itemsSerialization
          .filter((item) => item.type && item.type[0] === 'nypl:CheckinCardItem')
          .length
        this.addStatement('numCheckinCardItems', numCheckinCardItems)

        // Calculate number of "real" and checkin card items:
        const numItemsTotal = numCheckinCardItems + numRealSierraItems
        this.addStatement('numItemsTotal', numItemsTotal)

        const numAvailable = itemsSerialization
          .reduce((sum, item) => sum + (item.status && item.status.length > 0 && item.status[0].id === 'status:a' ? 1 : 0), 0)
        this.addStatement('numAvailable', numAvailable)

        // Get number of items that have parsed volumeRanges:
        const numItemVolumesParsed = itemsSerialization
          .filter((item) => utils.isArrayWithValues(item.volumeRange))
          .length
        this.addStatement('numItemVolumesParsed', numItemVolumesParsed)

        // Get number of items that have parsed dateRanges:
        const numItemDatesParsed = itemsSerialization
          .filter((item) => utils.isArrayWithValues(item.dateRange))
          .length
        this.addStatement('numItemDatesParsed', numItemDatesParsed)
      }))
    }
...
ResourceSerializer.sortableShelfMark = (shelfMark) => {
  ...
  const replace = (m0, fullMatch, label, labelWhitespace, labelText, number) => {
    ...
    return label
      ? `${label.toLowerCase()}${ResourceSerializer.zeroPadString(number)}`
 ...
 */
ResourceSerializer.zeroPadString = (s, padLen = 6) => utils.leftPad(s, padLen, '0')
