...
const resourcesProperties = {
  ...
  holdings: {
    ...
    properties: {
      checkInBoxes: {
        type: 'nested',
        properties: {
          copies: mappingTemplates.number,
          coverage: mappingTemplates.exactString,
          position: mappingTemplates.number,
          shelfMark: mappingTemplates.exactStringNotIndexed,
          status: mappingTemplates.exactString,
          type: mappingTemplates.exactString
        }
      },
      format: mappingTemplates.exactString,
      holdingStatement: mappingTemplates.exactString,
      identifier: {
        properties: {
          value: mappingTemplates.exactString,
          type: mappingTemplates.exactString,
          identifierStatus: mappingTemplates.exactString
        }
      },
      location: {
        properties: {
          code: mappingTemplates.exactString,
          label: mappingTemplates.exactString
        }
      },
      notes: mappingTemplates.exactString,
      physicalLocation: mappingTemplates.exactStringNotIndexed,
      shelfMark: mappingTemplates.exactString,
      uri: mappingTemplates.exactString
    }
  },
  idIsbn: mappingTemplates.exactString,
  idIsbn_clean: mappingTemplates.exactString,
  ...
  items: {
    ...
    properties: {
      ...
      enumerationChronology: mappingTemplates.exactString,
      enumerationChronology_sort: mappingTemplates.exactString,
      formatLiteral: mappingTemplates.exactString,
      ...
      m2CustomerCode: mappingTemplates.exactString,
      owner: mappingTemplates.entity,
      owner_packed: mappingTemplates.packed,
      physicalLocation: mappingTemplates.exactString,
      recapCustomerCode: mappingTemplates.exactString,
      ...
      shelfMark_sort: mappingTemplates.exactString,
  ...
  numElectronicResources: mappingTemplates.number,
  numItemVolumesParsed: mappingTemplates.number,
  numItemDatesParsed: mappingTemplates.number,
  numCheckinCardItems: mappingTemplates.number,
  numItemsTotal: mappingTemplates.number,
  ...
  parallelTitleAlt: mappingTemplates.fulltextFolded,
  parallelSubjectLiteral: mappingTemplates.fulltextFolded,
  parallelUniformTitle: mappingTemplates.fulltextFolded,
  parallelCreatorLiteral: mappingTemplates.fulltextWithRawFolded,
  parallelContributorLiteral: mappingTemplates.fulltextWithRawFolded,
...
const reindexResources = (source, dest) => {
  // The way URLs are parsed in Node changed, so the next time we need to
  // run a reindex using this script, the developer should take a look at
  // this code to ensure correctness:
  console.log('This has been deprecated pending a review of the code.')
  process.exit()

  // Build a part of the body config:
  const prepareConfigPart = (val) => {
    const part = {}
...
const reindexStatus = () => {
  return client().tasks.list({ actions: '*reindex', detailed: true }).then((resp) => {
    ...
    const uri = `https://${__connectionUri}/_tasks/${task.nodeTaskId}`
...
const prepareResourcesIndex = (indexName, deleteIfExists) => {
  ...
  const ensureDoesNotExist = (index) => {
  ...
  return ensureDoesNotExist(indexName).then(() => {
    ...
    return client().indices.create({
      ...
      return client().indices.putMapping({ index: indexName, type: 'resource', body })
    })
  })
}
...
const _indexGeneric = function (indexName, records, update) {
  const body = []
  ...
  return client().bulk({ body }).then((resp) => {
    return resp
...
const indexResources = function (indexName, records, update) {
...
const deleteRecord = function (index, type, id) {
  if (!id) throw new Error('deleteRecord needs an id: ', id)
  const h = { index, type, id }
    ...
    .then((list) => {
      return client().indices.stats({ index }).then((stats) => {
        return Object.keys(list).map((index) => {
          let aliases = list[index].aliases
...
const setAlias = (opts) => {
  ...
  return listIndexes().then((list) => {
    const activatedIndex = list.filter((ind) => ind.index === opts.index)[0]
    if (!activatedIndex) throw new Error(`Invalid index: ${opts.index}`)

    const previousIndex = list.filter((ind) => ind.aliases.indexOf(opts.alias) >= 0)[0]
...
const deleteIndex = (index) => {
  return listIndexes().then((list) => {
    ...
    const aliases = list.filter((ind) => ind.index === index)[0].aliases
...
const connected = () => {
  return Boolean(__connectionUri)
}
