...
class ResourceSerializer extends EsSerializer {
  serialize () {
    ...
    if (this.object.has(titleSpec.pred)) {
      var title = this.object.get(titleSpec.pred).object_literal
    ...
    bibFieldMapper.specFor('Language', (spec) => {
    ...
    bibFieldMapper.specFor('Resource type', (spec) => {
      this.object.each(spec.pred, (triple) => {
    ...
    // now add a bunch of properties that are identifiers:
    ; [
      'Identifier'
    ].forEach((name) => {
      bibFieldMapper.specFor(name, (spec) => {
        this.object.each(spec.pred, (triple) => {
...
class ResourceItemSerializer extends EsSerializer {
  serialize () {
    ...
    ; [
      ...
      itemFieldMapper.specFor(name, (spec) => {
        this.object.each(spec.pred, (triple) => {
          this.addStatement(spec.jsonLdKey, triple.object_id, triple.object_label)
        })
      })
    ...
    bibFieldMapper.specFor('Electronic location', (spec) => {
      this.object.each(spec.pred, (triple) => {
