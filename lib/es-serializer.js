  ...
  serialize () {
    return FieldMapping.initialize('resources').then((fieldMapping) => {
      ...
      if (this.object.has(locationPred)) {
        this.addStatement('deliveryLocation', this.object.get(locationPred).object_id, this.object.get(locationPred).object_label)
      }
      ...
      var suppressedPred = fieldMapping.predicateFor('Suppressed')
      if (this.object.has(suppressedPred)) {
        this.object.each(suppressedPred, (triple) => {
          this.addStatement('suppressed', triple.object_literal === 'true')
        })
      }
