...
class ResourceSerializer extends EsSerializer {
  ...
  serialize () {
    return FieldMapping.initialize('resources').then((fieldMapping) => {
      ...
      if (this.object.has('nypl:lccClassification')) {
        this.object.each('nypl:lccClassification', (triple) => {
          this.addStatement('lccClassification', triple.object_literal, triple.object_label)
        })
      }
      ...
      if (this.object.has(fieldMapping.predicateFor('Publisher literal'))) {
        this.object.each(fieldMapping.predicateFor('Publisher literal'), (triple) => {
          this.addStatement('publisherLiteral', triple.object_literal, triple.object_label)
        })
      }

      // A bunch of unanalyzed literals:
      ; ['Dimensions', 'Extent', 'Note', 'Place of publication'].forEach((field) => {
  ...
  serialize () {
    return FieldMapping.initialize('resources').then((fieldMapping) => {
      ...
      if (this.object.has(spec.predicate)) {
        ...
        // TODO: this is mis-mapped as an object; should be string
        // this.addStatement('prefLabel', this.object.literal(spec.predicate))
      }
