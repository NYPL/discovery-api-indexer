...
function getSchema (type) {
  ...
  return new Promise((resolve, reject) => {
    if (!process.env.NYPL_API_SCHEMA_URL) return reject(new Error('NYPL_API_SCHEMA_URL not set'))
    ...
    request(options, (error, resp, body) => {
      ...
      if (body.data && body.data.schema) {
        ...
        reject(new Error('Error fetching ' + type))
      }
    })
  })
}
...
function getSchemas (schemasToFetch) {
  ...
  return Promise.all(schemasToFetch.map((name) => getSchema(name)))
    .then((schemas) => {
      ...
      const failed = schemasToFetch.reduce((failed, schemaName) => {
