...
class IndexerRunner {
  ...
  run () {
    ...
    this.getQueryCount((count) => {
      ...
      this.currentStart = this.options.offset
      console.log('offset; ', this.options.offset)

      this.seekDelay = Math.min(20, 2 + this.perBot / 300)
  ...
  considerBuildingWorker () {
    ...
    // var limit = Math.min( 25000 ,  250000  -  275000───────────────────────────────────────────────────────────┐┌─Progress───────────────────────────────────────────┐

    console.log('var limit = Math.min(', this.perBot, ', ', this.jobCount, ' - ', this.currentStart)
    var limit = Math.min(this.perBot, Math.abs(this.jobCount - this.currentStart))
  ...
  buildWorker (start, limit) {
    ...
    worker.on('message', function (msg) {
      ...
      if (msg.log) {
        this.viz.log('Worker ' + worker.id + ': ' + msg.log)
      }
  ...
  init () {
    ...
    this.cluster.on('disconnect', (worker, code, signal) => {
      var remaining = Math.max(0, Object.keys(this.cluster.workers).length - 1)
