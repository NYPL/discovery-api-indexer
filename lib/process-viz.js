...
class ProcessViz {
  ...
  constructor () {
    ...
    var grid = new contrib.grid({rows: 12, cols: 12, screen: this.screen}) // eslint-disable-line new-cap
  ...
  updateActiveBotsTable () {
    ...
    this.active_bots_table.setData({headers: ['ID', 'Completed', 'Skipped', 'Total', '%', 'Status'], data: data})
    this.screen.render()
  }
  ...
  setStat (label, value) {
    ...
    var statsArray = Object.keys(this._stats).map((k) => {
