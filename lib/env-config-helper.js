...
function dbConnect (discoveryStoreOrm) {
  if (discoveryStoreOrm.connected()) return Promise.resolve()
  else {
    return kmsHelper.decryptDbCreds()
      .then((uri) => discoveryStoreOrm.connect(uri))
  }
}
...
function init (clients) {
  ...
  var initializations = []
  if (clients.discoveryStoreOrm) initializations.push(dbConnect(clients.discoveryStoreOrm))
